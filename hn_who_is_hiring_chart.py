...
<head>
  ...
  <style>
    ...
    body[data-theme="dark"] .meta {{ color: #94a3b8; }}
    ...
    body[data-theme="dark"] .methodology {{
      background: #0f172a;
      box-shadow: 0 8px 30px rgba(2, 6, 23, 0.6);
    }}
    body[data-theme="dark"] .methodology summary {{ color: #e2e8f0; }}
    body[data-theme="dark"] .methodology p,
    body[data-theme="dark"] .methodology li {{ color: #cbd5f5; }}
  </style>
</head>
<body>
  <div class="page">
  ...
  <div class="controls">
    <button id="modeToggle" type="button">Normalize</button>
  </div>
  <div class="tab-bar">
    <button class="tab-button active" data-tab="main">WIH comments</button>
    <button class="tab-button" data-tab="categories">Category share</button>
  </div>
  ...
  <script>
    ...
    const categoryColors = [
      "#f97316", "#0ea5e9", "#22c55e", "#ef4444", "#6366f1",
      "#14b8a6", "#eab308", "#ec4899", "#8b5cf6", "#10b981",
      "#a855f7", "#64748b"
    ];

    const baseLayout = {{
      margin: {{ t: 30, r: 30, l: 60, b: 80 }},
      xaxis: {{
        type: "date",
        tickformat: "%Y-%m",
        tickangle: -45,
    ...
    function formatValue(value, mode) {{
      ...
      if (mode === "normalized") return value.toFixed(2);
      ...
      return categoryOrder
        .map((category, idx) => {{
          ...
          return {{
            ...
            opacity: 0.85,
            customdata: y,
            hovertemplate: "%{{x|%Y-%m}}<br>" + category + ": %{{y:.1f}}%<br>Comments: %{{customdata}}<extra></extra>",
            name: category
          }};
        }})
        .filter(Boolean);
    ...
    function buildTraces(theme, mode) {{
      ...
      const markerColor = theme === "dark" ? "#7dd3fc" : "#1d4ed8";
      const trendColor = theme === "dark" ? "#c084fc" : "#7c3aed";
      return [
        {{
          x, y,
          type: "scatter",
          ...
          hovertemplate: isNorm
            ? "%{{x|%Y-%m}}<br>Comments/10k: %{{y:.2f}}<br>WIH comments: %{{customdata[0]}}<br>HN comments: %{{customdata[1]}}<extra></extra>"
            : "%{{x|%Y-%m}}<br>Comments: %{{y}}<br>HN comments: %{{customdata}}<extra></extra>",
          name: "Comments"
        }},
        {{
          x, y: trend,
          ...
          hovertemplate: "%{{x|%Y-%m}}<br>Trend: %{{y:.2f}}<extra></extra>",
          name: "12‑mo MA"
        }},
        {{
          ...
          type: "scatter",
          mode: "markers+text",
          marker: {{ color: "#22c55e", size: 9 }},
          text: stats.maxIdx >= 0 ? [`Max ${{formatValue(y[stats.maxIdx], mode)}}`] : [],
        ...
        {{
          ...
          text: stats.minIdx >= 0 ? [`Min ${{formatValue(y[stats.minIdx], mode)}}`] : [],
    ...
    function buildCategoryLayout(theme, mode) {{
      const isNorm = mode === "normalized";
    ...
    function buildLayout(theme, mode) {{
      const isNorm = mode === "normalized";
      const y = isNorm ? yNorm : yRaw;
      const stats = computeStats(y);
      const themeLayout = theme === "dark" ? darkTheme : lightTheme;
      return {{
        ...
        yaxis: {{
          ...(baseLayout.yaxis || {{}}),
          ...(themeLayout.yaxis || {{}}),
          title: isNorm ? "Comments per 10k HN comments" : "Comments",
          tickformat: isNorm ? ".2f" : undefined
        }},
        xaxis: {{
          ...(baseLayout.xaxis || {{}}),
          ...(themeLayout.xaxis || {{}}),
        ...
        annotations: [
          {{
            ...
            showarrow: false,
            font: {{ color: theme === "dark" ? "#cbd5f5" : "#475569", size: 12 }}
          }}
        ]
      }};
    }}
    ...
    function getPanelSize(id) {{
      ...
      return {{
        ...
        width: Math.max(640, width - 16)
      }};
    }}

    function renderCharts(theme) {{
      const mainLayout = buildLayout(theme, currentMode);
      ...
      const categoryLayout = buildCategoryLayout(theme, currentMode);
